...
#
#
# This file is part of TeleUp
#
# TeleUp is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
#
# TeleUp is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU General Public License for more details.
#
# You should have received a copy of the GNU General Public License
# along with TeleUp. If not, see <http://www.gnu.org/licenses/>.
...
class TeleUp():
    ...
    def buildHeader(self):
        ...
        header_dict['Referer']    = BASEWEB
        header_dict['Origin']     = BASEWEB
        ...
        if len(user) > 0:
            ...
            try:
                ...
                if custlink and 'username' in custlink and user.lower() == custlink['username'].lower():
                    ...
                    #login
                    loginlink = (self.net.http_POST(BASEURL + 'iphone/1/live/login', form_data={'username':user,'password':password,'device':'iphone'}, headers=header_dict).content.encode("utf-8").rstrip())
                    token     = re.findall(r'token=(.*?)"', loginlink, re.DOTALL)
                    if loginlink and token:
                ...
                if not self.channels:
                    ...
                    '''{u'app_name': u'preview', u'stream': u'00000WXYZustvnow', u'af': u'US', u'dvraction': u'add', u'callsign': u'WHTM', u'event_inprogress': 1, 
                        ...
                        u'dvrtimeraction': u'add', u'mediatype': u'EP', u'auth': None, u'stream_origin': u'dne.teleup.com', u'dvrtimertype': 0, u'scode': u'whtm', 
            ...
            for channel in self.recorded:
                if url == str(channel['scheduleid']):
                    try:
                        ...
                        '''{u'pr': u'll', u'domain': u'ilvc02.ll.teleup.com',u'stream': u'http://ilvc02.ll.teleup.com/ilv10/pr/xxl/smil:0B64AWHTMUSTVNOW/playlist.m3u8?', 
            ...
            for channel in self.channels:
                if url == self.grabChannelName(channel['stream_code']):
                    try:
                        ...
                        '''{u'app_name': u'preview', u'srsid': 1843, u'event_time': u'21:27:00', u'title': u'Kansas City Confidential', u'timemark': 1525471200, u'recordedon': u'May 4, 2018 17:27', u'xcdrappname': u'livehd', u'has_img': 1, 
                            u'updated': u'2018-05-04 13:53:34', u'episode_season': 0, u'actualremainingtime': 3949, u'dvrtimeraction': u'add', u'mediatype': u'MV', u'auth': 0, u'stream_origin': u'dne.ustvnow.com', u'dvrtimertype': 0, u'scode': 
                            u'flmsrl', u'guideheight': 130, u'prgsvcid': 72510, u'event_inprogress': 1, u'content_allowed': False, u'episode_title': u'', u't': 0, u'connectorid': u'MV000029920000', u'imgmark': u'live', u'dvraction': u'add', u'displayorder': 127, 
                            u'guideremainingtime': 3960, u'stream': u'00000WXYZteleup', u'af': u'US', u'callsign': u'FOR', u'scheduleid': 173991644, u'favoriteaction': u'add', u'seo_title': u'kansas-city-confidential', u'prg_img': u'v5/NowShowing/1843/p1843_p_v5_aa.jpg', 
                            u'title_10': u'Kansas', u'event_date': u'2018-05-04', u'stream_code': u'Films on Reel', u'description': u"Ex-convict (John Payne) framed for holdup done by policeman's (Preston Foster) gang.", u'details_uri': u'/info/films-on-reel/kansas-city-confidential/2018-05-04/21-27-00-utc', 
                            u'seo_name': u'films-on-reel', u'aksid': 1, u'episode_number': 0, u'orig_air_date': None, u'content_id': u'MV000029920000', u'img': u'images/FOR.png', u'streamname': u'00000WXYZteleup', u'synopsis': u'A robbery suspect tries to find the real mastermind.', u'edgetypes': 15, u'runtime': 5940, u'order': 1, u'ut_start': 1525469220}'''                                                                                     
                        if URL_TYPE == 'm3u8': stream = urllink['stream']
                        else: stream = (urllink['stream'].replace('smil:','mp4:').replace('TELEUP','TELEUP%d'%URL_QUALITY))
    ...
    def replaceToken(self, url, dvr):
        ...
        try:
            loginlink = (self.net.http_POST(BASEURL + 'iphone/1/live/login', form_data={'username':USER_EMAIL,'password':PASSWORD,'device':'iphone'}, headers=self.buildHeader()).content.encode("utf-8").rstrip())
            altToken = re.findall(r'token=(.*?)"', loginlink, re.DOTALL)[0]
            self.token = altToken
    ...
    def poolList(self, method, items):
        self.log("poolList, method = " + method.func_name)
        results = []
        if ENABLE_POOL:
            results = self.pool.imap_unordered(method, items)
            self.pool.close()
            self.pool.join()
