# -*- coding: utf-8 -*-
"""Kodi ListItem/artwork helpers for ZenRadio channels and Now Playing."""
...
def music_listitem(
    ...
    """Build the initial Kodi ListItem for a linear ZenRadio stream."""
    item = xbmcgui.ListItem(path=stream_url)

    tag = item.getMusicInfoTag()
    tag.setTitle(str(channel_name))
    tag.setArtist("ZenRadio")
    tag.setAlbum(f"ZenRadio — {channel_name}")
