...
def update_linear_metadata(client, player, state):
    ...
    try:
        ...
        if playing_file != expected_stream:
            xbmc.log(
                "[plugin.audio.zenradio] playing stream mismatch: "
    ...
    artist = current.get("display_artist") or "ZenRadio"
    album = f"ZenRadio — {state.get('channel_name') or channel}"
    ...
    try:
        ...
        xbmc.log(
            f"[plugin.audio.zenradio] unable to get playing item: {exc!r}",
            xbmc.LOGDEBUG,
        )
        ...
        try:
            ...
            xbmc.log(
                f"[plugin.audio.zenradio] unable to set playing path: {path_exc!r}",
                xbmc.LOGDEBUG,
            )
    ...
    try:
        ...
        if playing_file != expected_stream:
            xbmc.log(
                "[plugin.audio.zenradio] playing stream mismatch before metadata update: "
                f"getPlayingFile={safe_url_for_log(playing_file)!r}, "
                f"state_stream_url={safe_url_for_log(expected_stream)!r}",
                xbmc.LOGDEBUG,
            )
    ...
    xbmc.log(
        "[plugin.audio.zenradio] Now Playing updated: "
    ...
    while not monitor.abortRequested():
        ...
        try:
            ...
            try:
                ...
                xbmc.log(
                    f"[plugin.audio.zenradio] unable to read playing file: {exc!r}",
                    xbmc.LOGDEBUG,
                )
                continue
            ...
            if playing_file != expected_stream:
                xbmc.log(
                    "[plugin.audio.zenradio] playing stream mismatch in service loop: "
                    f"getPlayingFile={safe_url_for_log(playing_file)!r}, "
                f"state_stream_url={safe_url_for_log(expected_stream)!r}",
                    xbmc.LOGDEBUG,
                )
            ...
            xbmc.log(
                f"[plugin.audio.zenradio] metadata service API error: {exc}",
                xbmc.LOGDEBUG,
            )
        except Exception as exc:
            xbmc.log(
                f"[plugin.audio.zenradio] metadata service error: {exc!r}",
                xbmc.LOGDEBUG,
            )
