# -*- coding: utf-8 -*-
"""AudioAddict/ZenRadio API client used by the Kodi add-on.
...
ADDON_ID = "plugin.audio.zenradio"
...
class AudioAddictClient:
    """Small synchronous client for the AudioAddict API used by ZenRadio."""
    ...
    def __init__(self):
        ...
        self.http.headers.update({
            "User-Agent": f"Kodi ZenRadio/{version}",
    ...
    def _log(message, level=xbmc.LOGDEBUG):
        xbmc.log(f"[plugin.audio.zenradio] {message}", level)
    ...
    def channel_filters(self):
        """Return AudioAddict's current ZenRadio navigation filters/channels."""
        return self._get("/channel_filters")

    def favorite_channel_ids(self):
        """Return the current user's favourite ZenRadio channel IDs."""
        session = self._ensure_session()
    ...
    def current_track(self, channel_key, known_track_id=None):
        """Return Now Playing metadata for one linear ZenRadio channel.
