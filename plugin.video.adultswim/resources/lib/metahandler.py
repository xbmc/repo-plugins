...
class MetaData:
    ...
    def remove_database(self):
        ...
        try:
            ...
            if os.path.exists(self.videocache): os.remove(self.videocache)
    ...
    def get_meta(self, name, imdb_id='', tmdb_id='', year='', overlay=6):
        ...
        if imdb_id:
            ...
            meta = self._cache_lookup_by_id(imdb_id=imdb_id)
        elif tmdb_id:
            meta = self._cache_lookup_by_id(tmdb_id=tmdb_id)
        else:
    ...
    def _cache_save_video_meta(self, meta_group, name, overlay=6):
        ...
        try:
            ...
            for meta in meta_group:
                ...
                self.dbcur.execute(sql_insert, (meta['imdb_id'], meta['tvdb_id'], meta['title'], meta['year'], meta['cast'], meta['rating'], meta['duration'], meta['plot'], meta['mpaa'], meta['premiered'], meta['genre'], meta['studio'], meta['status'], meta['banner_url'], meta['cover_url'], meta['trailer_url'], meta['backdrop_url'], meta['overlay']))
