# -*- coding: utf-8 -*-
# Kodi Esquire TV Video Addon !!!!!!!!!!!!!!!!!! still need to add subtitles !!!!!
...
def getSources(fanart):
              ilist=[]
              urlbase = 'http://tv.esquire.com/now/'
              html = getRequest(urlbase)
              blob = re.compile('<div class="shows">(.+?)</div').search(html).group(1)
...
def getCats(cat_url):
              ...
              a = json.loads(html)
              a = a["tve_widgets"]["clone_of_latest_episodes"]["assets1"]
              for b in a:
...
def getShow(url, show_name):
            ...
            purl = 'http:'+purl+'&player=Esquire%20VOD%20Player%20%28Phase%203%29&format=Script&height=576&width=1024'
            ...
            u   = 'https://tveesquire-vh.akamaihd.net/i/prod/video/%s_,1696,1296,896,696,496,240,306,.mp4.csmil/master.m3u8?__b__=1000&hdnea=st=%s~exp=%s' % (url, str(unow), str(unow+60))
