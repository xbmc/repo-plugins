...
class SubtitlesContentHandler(xml.sax.ContentHandler):
    ...
    def startElement(self, name, attrs):
        if name == "tt:style":
          font_name = attrs.get("xml:id")
          font_wert = attrs.get("tts:color")
          if font_wert:
            self.schriften[font_name]=font_wert
            print  font_name +"="+ font_wert
        if name == "tt:p":
            self._startEntry(attrs.get("begin"), attrs.get("end"))
        elif name == "tt:span":
            self.color=attrs.get("style")
            self._startLine()
    ...
    def characters(self, content):
        if(self._line):
            self._result += "<font color="+self.schriften[self.color]+">"+content +"</font>"
    ...
    def _startEntry(self, begin, end):
        ...
        self._result += "\n"
        self._result += "0"+begin.replace('.', ',')[1:]
        self._result += " --> "
        self._result += end.replace('.', ',')[1:]
        self._result += "\n"
...
def download_subtitles(url, subtitles_dir):
    """Downloads and parses TTML subtitles from the given URL and saves it as br3.srt in the given subtitles directory.
    ...
    path = os.path.join(subtitles_dir, 'br3.srt')
    ...
    logger = logging.getLogger("plugin.video.br3.subtitles")
    try:
