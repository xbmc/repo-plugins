...
#TODO Integration of WatchedPlayer and ResumePlayer is messy. Need a solution that allows watch & resume without tightly coupling them.
import os
import sys
import re
import time
...
class PlayerLockException(LoggingException):
    """
    Exception raised when ResumePlayer fails to obtain a resume lock
    """
    pass
...
"""
class ResumePlayer(BasePlayer):
    def __init__( self, pid, live ):
        a = 0
"""
class ResumePlayer(BasePlayer):
    """
    An XBMC player object, for supporting resumable playback 
    """
    ...
    #def __init__( self, pid, live ):
    def init( self, pid, live, playerName, resumeEnabled = True ):
        xbmc.log(u"%s: ResumePlayer initialised (pid: %s, live: %s, playerName: %s)" % (self, pid, live, playerName), xbmc.LOGINFO)
        self.paused = False
        self.live = live
        self.pid = pid
    ...
    def _acquire_lock( self ):
        if os.path.isfile(ResumePlayer.RESUME_LOCK_FILE):
            raise PlayerLockException(u"Only one instance of %s can be run at a time. Please stop any other streams you may be watching before opening a new stream" % self.playerName)
        else:
            lock_fh = open(ResumePlayer.RESUME_LOCK_FILE, u'w')
            try:
    ...
    def _release_lock( self ):
        self_has_lock = False
        lock_fh = open(ResumePlayer.RESUME_LOCK_FILE)
        try:
        ...
        if self_has_lock:
            ...
            try:
                os.remove(ResumePlayer.RESUME_LOCK_FILE)
            except Exception, e:
                xbmc.log(u"Error removing %s resume lock file! (%s)" % (self.playerName, e), xbmc.LOGWARNING)
    ...
    def force_release_lock():
        """
        If something goes wrong and the lock file is present after the ResumePlayer object that made it dies,
    ...
    def run_heartbeat( self ):
        ...
        if not self.live and not self.cancelled.is_set():
            ...
            xbmc.log(u"%s ResumePlayer: current_seek_time %s" % (self, self.current_seek_time), level=xbmc.LOGDEBUG)
    ...
    def save_resume_point( self, resume_point ):
        ...
        ResumePlayer.save_resume_file(resume, dates_added)
    ...
    def load_resume_file():
        ...
        if not ResumePlayer.resume:
            # Load resume file
            ResumePlayer.resume = {}
            ResumePlayer.dates_added = {}
            if os.path.isfile(ResumePlayer.RESUME_FILE):
                xbmc.log(u"ResumePlayer: Loading resume file: %s" % (ResumePlayer.RESUME_FILE), xbmc.LOGINFO)
                with open(ResumePlayer.RESUME_FILE, 'rU') as resume_fh:
                ...
                try: days_to_keep = int(ResumePlayer.ADDON.getSetting(u'resume_days_to_keep'))
                ...
                ResumePlayer.resume = dict(pid_to_resume_point_map)
                ResumePlayer.dates_added = dict(pid_to_date_added_map)
                xbmc.log(u"ResumePlayer: Found %d resume entries" % (len(ResumePlayer.resume.keys())), xbmc.LOGINFO)
                
        return ResumePlayer.resume, ResumePlayer.dates_added

    @staticmethod
    def delete_resume_point(pid_to_delete):
        xbmc.log(u"ResumePlayer: Deleting resume point for pid %s" % pid_to_delete, xbmc.LOGINFO)
        resume, dates_added = ResumePlayer.load_resume_file()
    ...
    def save_resume_file(resume, dates_added):
        ...
        ResumePlayer.resume = resume
        ResumePlayer.dates_added = dates_added
        
        str = u""
        xbmc.log(u"ResumePlayer: Saving %d entries to %s" % (len(resume.keys()), ResumePlayer.RESUME_FILE), xbmc.LOGINFO)
        resume_fh = open(ResumePlayer.RESUME_FILE, u'w')
        try:
