...
class Provider(kodion.AbstractProvider):
    ...
    def __init__(self):
        ...
        self._local_map = {'7tv.popular_shows': 30500,
    ...
    def on_search(self, search_text, context, re_match):
        ...
        result = []
        json_data = context.get_function_cache().get(FunctionCache.ONE_MINUTE, self._get_client(context).search, self._get_client(context).API_V2,
                                                     search_text)
    ...
    def _get_channel_highlights(self, context, re_match):
        ...
        if category is None:
            ...
            json_data = context.get_function_cache().get(FunctionCache.ONE_MINUTE * 5, self._get_client(context).get_homepage,
    ...
    def _play(self, context, re_match):
        ...
        return VideoItem(video_id, video_url)
    ...
    def _load_format_content(self, context, channel_id, format_id, return_cached_only=False):
        ...
        if return_cached_only:
            data = context.get_function_cache().get_cached_only(self._get_client(context).get_format_content, self._get_client(context).API_V2,
    ...
    def _get_format_content(self, context, re_match):
        ...
        next_page_test = context.get_function_cache().get(FunctionCache.ONE_HOUR, self._get_client(context).get_format_videos,
                                                          self._get_client(context).API_V2, channel_id, format_id, clip_type,
                                                          page=page + 1)
        ...
        if page == 1:
            ...
            video_list = []
            episodes = context.get_function_cache().get(FunctionCache.ONE_HOUR, self._get_client(context).get_format_videos,
                                                        self._get_client(context).API_V2, channel_id, format_id, clip_type,
                                                        page=page)
    ...
    def _get_channel_formats(self, context, re_match):
        ...
        channel_id = re_match.group('channelid')
        json_data = context.get_function_cache().get(FunctionCache.ONE_DAY, self._get_client(context).get_formats, self._get_client(context).
                                                     API_V2, channel_id)
        screen = json_data.get('screen', {})
        screen_objects = screen.get('screen_objects', {})
        ...
        for screen_object in screen_objects:
            ...
            context_menu = [(context.localize(kodion.constants.localize.FAVORITES_ADD),
                             build_in_functions.run_plugin_add_to_favs(context, format_item))]
    ...
    def _latest_video(self, context, re_match):
        ...
        json_data = context.get_function_cache().get(FunctionCache.ONE_MINUTE * 5, self._get_client(context).get_new_videos,
    ...
    def _screen_object_to_item(self, context, screen_object, show_format_title=False):
        ...
        if screen_object_type == 'video_item_date_no_label' or screen_object_type == 'video_item_date' \
            ...
            context_menu = [(context.localize(kodion.constants.localize.WATCH_LATER),
                             build_in_functions.run_plugin_add_to_watch_later(context, video_item))]
