...
def getSources(fanart):
        ...
        a = re.compile('<li id="\/content\/.+?href="(.+?)">(.+?)<.+?</li>').findall(html)
        for url,name in a:
              mode = 'GC'
              url = url.replace('.html','.grid.1.html')
...
def getCats(gsurl,catname):
        ilist = []
        html  = getRequest('http://www.cookingchanneltv.com%s' % gsurl)
        c     = re.compile('<li id=.+?src="(.+?)".+?"name">(.+?)<.+?capt">(.+?)<.+?</li').findall(html)
        for img, name, plot in c:
              name = cleanname(name.decode(UTF8))
              url = img.rsplit('/',1)[1]
              url = url.split('_',1)[0]
              url = '%s/%s' % (url[0:4], url)
              if int(addon.getSetting('vid_res')) == 0 : speed = '3'
              else: speed = '6'
              url = 'http://sni-i.akamaihd.net/i/%s_,%s,.mp4.csmil/master.m3u8?__b__=800&' % (url, speed)
              mode = 'GL'
              u = '%s?url=%s&name=%s&mode=%s' % (sys.argv[0],qp(url), qp(name), mode)
