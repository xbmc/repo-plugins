...
class myAddon(t1mAddon):
  ...
  def getAddonMenu(self,url,ilist):
    ...
    basehtml = self.getRequest('http://www.bravotv.com/full-episodes')
    cats = re.compile('<div class="views-row">.+?href="(.+?)".+?title="(.+?)".+?data-src="(.+?)".+?</article',re.DOTALL).findall(basehtml)
    dirty = False
    ...
    for i, (url, name, thumb) in list(enumerate(cats, start=1)):
      ...
      try:   (name, thumb, fanart, infoList) = meta['shows'][url]
        ...
        html = self.getRequest(BRAVOBASE % epis[len(epis)-1])
        try:
