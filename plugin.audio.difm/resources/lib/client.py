...
class AudioAddictClient:
    ...
    def __init__(self):
        ...
        self.http.headers.update({
            "User-Agent": f"Kodi DI.FM/{version}",
    ...
    def current_track(self, channel_key, known_track_id=None):
        """Return Now Playing metadata for one linear DI.FM channel.
    ...
    def resolve_stream(self, channel_key):
        ...
        # A timeout/connection failure does not prove Kodi cannot play the
        # stream. Let Kodi try the first inconclusive candidate.
        if inconclusive:
