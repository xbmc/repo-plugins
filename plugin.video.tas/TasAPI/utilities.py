...
def download_cache():
	...
	filename = 'images.zip'
	archive = xbmc.translatePath(os.path.join( addonsavepath, 'cache', filename))
	remoteaddr = 'http://insayne-projects.googlecode.com/files/'
	try:
...
def unzip_downloaded_cache():
	filename = 'images.zip'
	archive = xbmc.translatePath(os.path.join( addonsavepath, 'cache', filename))
	outdir = xbmc.translatePath(os.path.join( addonsavepath, 'cache', 'images'))
	pDialog = xbmcgui.DialogProgress()
	dia_title = common.get_lstring(32000)
