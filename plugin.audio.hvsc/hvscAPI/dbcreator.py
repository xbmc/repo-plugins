...
def dbcreate(csv_db):
	...
	for row in csvfile_content:
		...
		if (count!=1):
			...
			pDialog.update(float(percent), dia_l1, dia_l2, dia_l3)
...
def unzip_downloaded_cache(archive):
	outdir = xbmc.translatePath(os.path.join( 'special://profile/addon_data/plugin.audio.hvsc/', 'temp'))
	pDialog = xbmcgui.DialogProgress()
	dia_title = common.get_lstring(50016)
...
def get_csv_link():
	...
	download(downloadurl, "special://profile/addon_data/plugin.audio.hvsc/temp")
