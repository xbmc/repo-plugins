...
class Provider(kodion.AbstractProvider):
    ...
    def _rss_to_items(self, context, rss, query=None, year=None, month=None):
        result = []
        ...
        for event, item in ET.iterparse(rss_stream):
            if item.tag == 'item':
                ...
                video_id = re.search(r"(?P<video_id>\d+)", guid)
                if video_id:
                    video_id = video_id.group('video_id')
                    uri = context.create_uri(['play'], {'video_id': video_id, 'url': guid})
    ...
    def _on_play(self, context, re_match):
        ...
        video_url = client.get_video_stream(video_id, url, quality=video_quality)
