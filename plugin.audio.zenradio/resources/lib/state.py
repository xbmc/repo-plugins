...
def load_state():
    ...
    try:
        ...
        xbmc.log(
            f"[plugin.audio.zenradio] unable to read playback state: {exc!r}",
            xbmc.LOGWARNING,
        )
    return {}
...
def save_state(data):
    ...
    try:
        ...
        xbmc.log(
            f"[plugin.audio.zenradio] unable to save playback state: {exc!r}",
            xbmc.LOGWARNING,
        )
...
def clear_state():
    ...
    try:
        ...
        xbmc.log(
            f"[plugin.audio.zenradio] unable to clear playback state: {exc!r}",
            xbmc.LOGWARNING,
        )
...
def update_state_if_current(stream_url, channel_key, updates):
    ...
    try:
        ...
        xbmc.log(
            f"[plugin.audio.zenradio] unable to update playback state: {exc!r}",
            xbmc.LOGWARNING,
        )
        return False
...
def _write_state_atomic(path, data):
    ...
    try:
        ...
        try:
            ...
            xbmc.log(
                "[plugin.audio.zenradio] unable to restrict playback state "
                f"permissions: {exc!r}",
                xbmc.LOGWARNING,
            )
