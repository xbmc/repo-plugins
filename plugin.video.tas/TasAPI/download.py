...
if not sys.argv[1]=="":
	...
	if sys.argv[1]=="nofolder":
		...
		if not sys.argv[2]=="":
			...
			line = "<url>" + sys.argv[1] + "</url><local>" + sys.argv[2] + "</local>\r\n"
			queuefile = xbmc.translatePath(os.path.join(addonsavepath, "Queue.txt"))
			queuestatus = xbmc.translatePath(os.path.join(addonsavepath, "QueueStatus"))
			
			# If the Queue File doesnt exist, we just create it
			filecheck = os.path.isfile(queuefile)
			if filecheck==False:
				queue = open(queuefile,'w')
				queue.close()
			# We open it in read mode, to read the old content
			queue = open(queuefile,'r')
			old = queue.read()
			# We create the new text to be the file
			new = old + line
			queue.close()
			# Now we open it in write mode and write to it
			queue = open(queuefile,'w')
			queue.write(new)
			queue.close()
			# And we are done with that... launching the script.. i hope it will recognize its runnign and not run twice
			filecheck = os.path.isfile(queuestatus)
			if filecheck==False:
				print "QUEUE IS NOT RUNNING SO WE RUN IT"
				script = os.path.join( addonpath, "TasAPI" ,"bgas.py" )
				xbmc.executebuiltin( "RunScript(" + script + ")" )	
			else:
				print "QUEUE IS RUNNING SO I WONT DO CRAP"
