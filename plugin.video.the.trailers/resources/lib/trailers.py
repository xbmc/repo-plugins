...
from utils import get_legal_filepath
    ...
    def _parse_categories( self, xmlSource, category ):
        try:
            ...
            for title, icon, thumb in categories:
                ...
                if ( thumb is None or not xbmcvfs.exists( thumb ) ):
                    thumb = icon
                # create our listitem
    ...
    def _parse_trailers( self, xmlSource, category ):
        try:
            ...
            for trailer in trailers:
                ...
                resolution = "1080"
                ok = self._add_video( { "title": title, "duration": duration, "mpaa": mpaa, "studio": studio, "postdate": postdate, "releasedate": releasedate, "copyright": copyright, "director": director, "plot": plot, "cast": actors, "genre": " / ".join( genres ), "poster": poster, "trailer": trailer_url, "size": size, "resolution": resolution } )
...
class Main:
    ...
    def _get_xml_source( self ):
        try:
            ...
            for source in ( "current.xml", "current_480p.xml", "current_720p.xml", "newest.xml", "newest_480p.xml", "newest_720p.xml",):
            #for source in ( "newest.xml", "newest_480p.xml", "newest_720p.xml", "current.xml", "current_480p.xml", "current_720p.xml", ):
