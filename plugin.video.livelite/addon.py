...
def switcher():
    ...
    groups = [localisedstr(30021)] + constructor()[1]
...
def list_items():
    ...
    root_menu = [
        ...
        ,
        {
            'title': localisedstr(30015).format(localisedstr(30021) if addon.getSetting('group') == 'ALL' else addon.getSetting('group').decode('utf-8')),
    ...
    for item in items:
        ...
        if addon.getSetting('youtube') == 'true' and item['url'].startswith('plugin://plugin.video.youtube/play/?video_id='):
            _url_ = '{0}?action=play&url={1}'.format(addon_url, item['url'])
        if addon.getSetting('youtube') == 'false' and item['url'].startswith('plugin://plugin.video.youtube/play/?video_id='):
            _url_ = item['url']
