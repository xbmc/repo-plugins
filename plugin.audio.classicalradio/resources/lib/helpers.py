# -*- coding: utf-8 -*-
"""Kodi ListItem/artwork helpers for ClassicalRadio channels and Now Playing."""
...
def music_listitem(
    ...
    """Build the initial Kodi ListItem for a linear ClassicalRadio stream."""
    item = xbmcgui.ListItem(path=stream_url)

    tag = item.getMusicInfoTag()
    tag.setTitle(str(channel_name))
    tag.setArtist("ClassicalRadio")
    tag.setAlbum(f"ClassicalRadio — {channel_name}")
