...
class myAddon(t1mAddon):
  ...
  def getAddonMenu(self,url,ilist):
   ...
   url = 'http://www.slice.ca/video/'
   html = self.getRequest(url)
   a = re.compile('drawerTitle: "(.+?)".+?categories: "(.+?)"',re.DOTALL).findall(html)
   for name, url in a:
  ...
  def getAddonEpisodes(self,url,ilist):
    ...
    url = 'http://feed.theplatform.com/f/dtjsEC/5tUJLgV2YNJ5?form=siteSettingsJSON&byCategories=%s' % url
    html  = self.getRequest(url)
    a = json.loads(html)['items']
    for b in a:
      url = b['id']
      url = url.rsplit('/',1)[1]
      url = 'http://feed.theplatform.com/f/dtjsEC/5tUJLgV2YNJ5?form=siteSettingsJSON&byId=%s' % url
      html = self.getRequest(url)
      b = json.loads(html)['items'][0]
      url = b['content'][0]['url']
      name    = h.unescape(b['title'])
      thumb   = b['thumbnails'][0]['url']
      fanart = thumb
      infoList = {}
      infoList['Duration']    = int(float(b['content'][0]['duration']))
      infoList['Title']       = name
      try: infoList['Studio']      = b['network']
      except: pass
      infoList['Date']        = datetime.datetime.fromtimestamp(b['pubDateConverted']/1000).strftime('%Y-%m-%d')
      infoList['Aired']       = infoList['Date']
      infoList['Year']        = int(infoList['Date'].split('-',1)[0])
      try:    infoList['MPAA'] = b['rating']
      except: infoList['MPAA'] = None
      try:    infoList['Episode'] = int(b['episode'])
      except: infoList['Episode'] = None
      try:    infoList['Season']  = int(b['season'])
      except: infoList['Season']  = 1
      infoList['Plot']        = h.unescape(b["description"])
      infoList['TVShowTitle'] = b['show']
