# -*- coding: utf-8 -*-
"""Kodi directory/plugin entry point for the ZenRadio add-on."""
...
def log(msg, level=xbmc.LOGINFO):
    xbmc.log(f"[plugin.audio.zenradio] {msg}", level)
...
def run():
    ...
    try:
        ...
        xbmcgui.Dialog().ok("ZenRadio", str(exc))
        try:
            xbmcplugin.endOfDirectory(
        ...
        xbmcgui.Dialog().ok("ZenRadio", t(32031))
        try:
            xbmcplugin.endOfDirectory(
                HANDLE, succeeded=False, cacheToDisc=False
            )
