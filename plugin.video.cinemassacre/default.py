...
try:
    ...
    def showEpisodeYoutube(self, videoItem):
      ...
      if videoItem.find('?') > -1:
        ...
        if r:
          videoID = r[-1]
      if videoID:
        retUrl = 'plugin://plugin.video.youtube/?action=play_video&videoid=' + videoID
        return retUrl
  ...
  dialog.notification('Error:', 'line: ' + str(lineNo) + ', ' + str(ex), xbmcgui.NOTIFICATION_ERROR)

  doSubmit = dialog.yesno('Please Report This Error:', 'An Error was encountered on line: ' + str(lineNo), str(ex), 'Would you like to submit this error to the author?')
  ...
  if doSubmit:
    try:
      ...
      uuid = hashlib.md5(sysname + nodename + release + version + machine).hexdigest()
    except:
