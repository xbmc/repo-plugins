...
class indexer:
    ...
    def movies(self, url):
        try:
            ...
            if self.getTraktCredentialsInfo() == True:
                for i in self.list:
                    i.update({'cm': [
                    {'title': 30501, 'query': {'action': 'collectMovie', 'url': i['trakt']}},
                    {'title': 30502, 'query': {'action': 'watchlistMovie', 'url': i['trakt']}},
                    {'title': 30503, 'query': {'action': 'newlistMovie', 'url': i['trakt']}},
                    {'title': 30504, 'query': {'action': 'listMovie', 'url': i['trakt']}}
                    ]})

            for i in self.list: i.update({'nextlabel': 30500, 'nextaction': 'movies'})
    ...
    def tvshows(self, url):
        try:
            ...
            if self.getTraktCredentialsInfo() == True:
                for i in self.list:
                    i.update({'cm': [
                    {'title': 30501, 'query': {'action': 'collectTVShow', 'url': i['trakt']}},
                    {'title': 30502, 'query': {'action': 'watchlistTVShow', 'url': i['trakt']}},
                    {'title': 30503, 'query': {'action': 'newlistTVShow', 'url': i['trakt']}},
                    {'title': 30504, 'query': {'action': 'listTVShow', 'url': i['trakt']}}
                    ]})

            for i in self.list: i.update({'nextlabel': 30500, 'nextaction': 'tvshows'})
    ...
    def authTrakt(self):
        try:
            if self.getTraktCredentialsInfo() == True:
                if control.yesnoDialog(control.lang(30181).encode('utf-8'), control.lang(30182).encode('utf-8'), '', 'Trakt'):
            ...
            result = json.loads(result)
            verification_url = (control.lang(30183) + '[COLOR skyblue]%s[/COLOR]' % result['verification_url']).encode('utf-8')
            user_code = (control.lang(30184) + '[COLOR skyblue]%s[/COLOR]' % result['user_code']).encode('utf-8')
    ...
    def traktLists(self, url, action):
        try:
            if action in ['newlistMovie', 'newlistTVShow']:
                ...
                except: return control.infoDialog(control.lang(30190).encode('utf-8'), heading=head, icon=icon)
                ...
                t = control.lang(30186).encode('utf-8')
            ...
            if url in ids:
                yes = control.yesnoDialog(control.lang(30187).encode('utf-8'), control.lang(30188).encode('utf-8'), '', 'Trakt')
                if not yes: return
                call = remove
            else:
                call = add
            ...
            info = control.lang(30189) if not result == None else control.lang(30190)
