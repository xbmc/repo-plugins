...
def getRequest(url, user_data=None, headers = defaultHeaders , alert=True, striplf = True):
              ...
              if (not (str(url).endswith('.zip'))) and (striplf == True):
...
def getShows(gsurl,catname):
        ...
        c     = re.compile("<A class='imageBox' href='(.+?)'.+?src='(.+?)'.+?title'>(.+?)<.+?description'>(.+?)<.+?</A").findall(html)
...
def getEpis(gsurl,catname):
        ...
        gsurl = uqp(gsurl)
        html  = getRequest('http://tubitv.com%s' % gsurl)
        c     = re.compile("<A class='imageBox' href='(.+?)'.+?src='(.+?)'.+?Title'>(.+?)<.+?</A").findall(html)
...
def getLink(url,vidname):
        ...
        try:
            a = re.compile("<script>apu='(.+?)'").search(html).group(1)
        except:
        ...
        try:
          ...
          data = getRequest(u)
          data = data+'#'
          urls = re.compile('BANDWIDTH=[0-9]*(.+?)#').findall(data)
        ...
        try:
           suburl = re.compile("sbt=(.+?);").search(html).group(1)
           if suburl != "undefined":
        ...
        if (suburl != "undefined") and ('.srt' in suburl) and (addon.getSetting('sub_enable') == "true"):
                 ...
                 captions = getRequest(suburl, striplf = False)
                 if captions != "":
