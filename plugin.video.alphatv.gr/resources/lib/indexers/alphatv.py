...
class indexer:
    ...
    def item_list_2(self, url):
        try:
            try: base_link = re.findall('(http(?:s|)://.+?)/', url)[0]
            except: base_link = self.base_link
        ...
        for item in items:
            try:
                title = client.parseDOM(item, 'div', attrs = {'class': 'itemtitle'})[0]
                try: title = client.parseDOM(title, 'span')[0]
                except: pass
                if title == '': raise Exception()
                title = client.replaceHTMLCodes(title)
                title = title.encode('utf-8')

                try: tvshowtitle = client.parseDOM(item, 'div', attrs = {'class': 'showtitle'})[0]
                except: tvshowtitle = title
