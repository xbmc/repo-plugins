...
ADDON_PATH: Final = ADDON.getAddonInfo("path")
...
class SearchDialog(xbmcgui.WindowXMLDialog):
    ...
    def run(cls, platforms) -> None:
        ...
        if not (context := dialog.getProperty("context")):
            return
        ...
        if result := zophar.search(context, platform):
            add_gamelist(result)
            return xbmcplugin.endOfDirectory(PLUGIN_HANDLE)

        xbmcgui.Dialog().ok(i18n(283), i18n(284))


def build_submenu(menu: str):
    menu_items, platforms = zophar.home()

    if menu == "Search":
        return SearchDialog.run(platforms)
...
def run() -> None:
    path, menu = arg("path"), arg("menu")

    if menu:
        return build_submenu(menu)
