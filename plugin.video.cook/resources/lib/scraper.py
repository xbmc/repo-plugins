...
class myAddon(t1mAddon):
  ...
  def getAddonMenu(self,url,ilist):
      html = requests.get('https://www.cookingchanneltv.com/videos/players/full-episodes-player', headers=self.defaultHeaders).text
      a = re.compile('<div class="m-MediaBlock o-Capsule__m-MediaBlock m-MediaBlock--playlist">.+?href="(.+?)".+?data-src="(.+?)".+?HeadlineText">(.+?)<.+?</div',re.DOTALL).findall(html)
  ...
  def getAddonEpisodes(self,url,ilist):
      ...
      for c in vids:
          ...
          thumb = ''.join(['https://www.cookingchanneltv.com',thumb])
          fanart = thumb
