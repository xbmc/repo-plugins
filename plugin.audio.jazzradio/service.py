...
def update_linear_metadata(client, player, state):
    ...
    try:
        ...
        if playing_file != expected_stream:
            xbmc.log(
                "[plugin.audio.jazzradio] playing stream mismatch: "
    ...
    try:
        ...
        if playing_file != expected_stream:
            xbmc.log(
                "[plugin.audio.jazzradio] playing stream mismatch before metadata update: "
                f"getPlayingFile={safe_url_for_log(playing_file)!r}, "
                f"state_stream_url={safe_url_for_log(expected_stream)!r}",
                xbmc.LOGDEBUG,
            )
...
def main():
    ...
    while not monitor.abortRequested():
        ...
        try:
            ...
            if playing_file != expected_stream:
                xbmc.log(
                    "[plugin.audio.jazzradio] playing stream mismatch in service loop: "
                    f"getPlayingFile={safe_url_for_log(playing_file)!r}, "
                f"state_stream_url={safe_url_for_log(expected_stream)!r}",
                    xbmc.LOGDEBUG,
                )
