...
class myAddon(t1mAddon):
  ...
  def getAddonMenu(self,url,ilist):
      ...
      html = self.getRequest('http://tbd.com')
      url = re.compile("'file':"+' "(.+?)"', re.DOTALL).search(html).group(1)
      thumb  = self.addonIcon
      fanart = self.addonFanart
      d = datetime.datetime.utcnow()
      now = calendar.timegm(d.utctimetuple())
      html = self.getRequest('http://mobilelistings.tvguide.com/Listingsweb/ws/rest/airings/20534/start/'+str(now)+'/duration/20160?channelsourceids=17075%7C13.2&formattype=json')
      a = json.loads(html)
      for b in a[:10]:
         infoList = {}
         b = b['ProgramSchedule']
