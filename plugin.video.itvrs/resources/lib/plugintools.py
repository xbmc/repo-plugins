...
def get_params():
    ...
    if param_string:
        split_commands = param_string[param_string.find('?')
            + 1:].split('&')
...
def read_body_and_headers(
    ...
    # one of the two cookie handling modules
        ...
        if cookielib is not None:
            ...
            if not follow_redirects:
                opener = \
                    urllib2.build_opener(urllib2.HTTPHandler(debuglevel=http_debug_log_enabled),
    ...
    for header in headers:
        _log('read_body_and_headers header %s=%s' % (str(header[0]),
             str(header[1])))
    ...
    fin = time.clock()
    _log('read_body_and_headers Downloaded in %d seconds ' % (fin
         - inicio + 1))
...
def add_item(
    ...
    _log('add_item action=[' + action + '] title=[' + title + '] url=['
         + url + '] thumbnail=[' + thumbnail + '] fanart=[' + fanart
         + '] show=[' + show + '] episode=[' + episode + '] extra=['
         + extra + '] page=[' + page + '] isPlayable=['
         + str(isPlayable) + '] folder=[' + str(folder) + ']')
    ...
    if url.startswith('plugin://'):
        ...
        xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),
                                    url=itemurl, listitem=listitem,
                                    isFolder=folder)
    elif isPlayable:
        listitem.setProperty('Video', 'true')
        listitem.setProperty('IsPlayable', 'true')
        itemurl = \
            '%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s' \
            % (
            sys.argv[0],
            action,
...
def direct_play(url):
    ...
    xlistitem.setArt({'thumb': 'DefaultVideo.png',
...
def message(text1, text2='', text3=''):
    _log("message text1='" + text1 + "', text2='" + text2 + "', text3='"
          + text3 + "'")
...
def message_yes_no(text1, text2='', text3=''):
    _log("message_yes_no text1='" + text1 + "', text2='" + text2
         + "', text3='" + text3 + "'")
