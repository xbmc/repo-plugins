...
__date__ = '05-25-2013'
__version__ = '0.0.1'
settings = xbmcaddon.Addon(id='plugin.video.testtube')
...
def build_sub_directory(url, name, slug, offset, daily_info):
	...
	if slug != 'None':
		try:
			...
			fanart = 'http://videos.testtube.com/revision3/images/shows/%s/%s_hero.jpg' % (slug, slug)
...
def build_networks_directory():
	html = getUrl('http://testtube.com/networks')
...
def build_networks_sub_directory(url, offset):
	saveurl = url
	html = getUrl('http://testtube.com' + url + '/episodePage?limit=25&offset=' + str(offset * 25))
	...
	for item in networkItem:
		...
		name = common.parseDOM(meta, "div", attrs = { "class": "title" })[0].encode('ascii', 'ignore')
...
def get_video(url, name, plot, studio, episode, thumb, date):
	if '{' in url:
		...
		oembed = getUrl('http://testtube.com/api/oembed/?url=http://testtube.com/%s/&format=json' % url)
	...
	if settings.getSetting('download') == 'true':
		...
		infoLabels = { "Title": name, "Studio": 'TestTube: ' + studio, "Plot": plot, "Episode": int(episode), "Aired": date  }
